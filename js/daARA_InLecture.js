...
function startLecture() {
  getNameFromS3();
  document.getElementById("imKING").style.backgroundImage =
    "url(./img/le_back.jpg)";
  ...
  //emotionLog 초기화
  const text =
    "[" + month + "월 " + day + "일 " + class_name + " 과목 감정인식 로그]\n";
  fs.appendFileSync(__dirname + "\\emotion.log", "\ufeff" + text, {
    encoding: "utf8",
  });
  show("Page2", "Page1");
}
...
async function getNameFromS3() {
  try {
    ...
    for (var i = 0; i < data["Contents"].length; i++) {
      ...
      if (
        getName == "예상 화면.png" ||
        getName == "screenshot.png" ||
        getName.length == 0
      )
        continue; //S3 Bucket에 올라가는 캡쳐본
      ...
      var text = document.createTextNode(
        uncheckedFaces[uncheckedFaces.length - 1]
      );
...
async function takeScreenShot() {
  ...
  client.detectFaces(params_2, function (err, response) {
    ...
    if (err) {
      ...
      var sorted_keys = Object.keys(map).sort(function (a, b) {
        return map[b] - map[a];
      });
      ...
      if (map[sorted_keys[0]] != 0) {
        document.getElementById("firstEmotion").value =
          "🥇" + sorted_keys[0] + " " + map[sorted_keys[0]] + "명";
      }
      if (map[sorted_keys[1]] != 0) {
        document.getElementById("secondEmotion").value =
          "🥈" + sorted_keys[1] + " " + map[sorted_keys[1]] + "명";
      }
      if (map[sorted_keys[2]] != 0) {
        document.getElementById("thirdEmotion").value =
          "🥉" + sorted_keys[2] + " " + map[sorted_keys[2]] + "명";
      }
    } // if
  });
}
...
stopBtn.onclick = (e) => {
  ...
  var options = {
    title: "출석파일 다운로드",
    filters: [{ name: "엑셀파일", extensions: ["xls"] }],
...
function buildTable() {
  ...
  for (var i = 0; i < checkedFaces.length; i++) {
    var row = `<tr> <td>${
      today.getMonth() + 1 + "월" + today.getDate() + "일"
    }</td> <td>${checkedFaces[i]}</td> <td>${"O"}</td> </tr>`;
    table.innerHTML += row;
  }
  for (var i = 0; i < uncheckedFaces.length; i++) {
    var row = `<tr> <td>${
      today.getMonth() + 1 + "월" + today.getDate() + "일"
    }</td> <td>${uncheckedFaces[i]}</td> <td>${"X"}</td> </tr>`;
    table.innerHTML += row;
  }
}
